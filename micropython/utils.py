...
# NEW: load persisted WORDPRESS_API_URL (used at boot + before uploads)
def load_persisted_wordpress_api_url():
    try:
        path = getattr(settings, 'WORDPRESS_API_URL_FILE', settings.LOG_DIR + '/wordpress_api_url.txt')
        val = read_text(path, '').strip()
        if val:
            settings.WORDPRESS_API_URL = val
            # Sync wprest module variable if present
            try:
                import wprest as _w
...
# Ensure URL loaded early (safe repeated calls)
try:
    load_persisted_wordpress_api_url()
except Exception:
    pass
