...
async def connectToWifiNetwork():
    ...
    retries = int(getattr(settings, 'WIFI_CONN_RETRIES', 10))
    backoff_s = int(getattr(settings, 'WIFI_BACKOFF_S', 2))
...
async def check_internet_connection(url=None):
    ...
    try:
        ...
        return bool(r) and getattr(r, 'status_code', 0) in (200, 204, 301, 302)
    except Exception:
        return False
    finally:
        try:
