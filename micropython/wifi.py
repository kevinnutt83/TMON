...
def get_settings():
    ...
    try:
        ...
        class _SettingsProxy:
            ...
            net_wifi_MAC = None
            net_wifi_IP = None
            MCU_TYPE = "esp32"  # Default to esp32
...
# Conditional import based on MCU_TYPE
s = get_settings()
MCU_TYPE = getattr(s, 'MCU_TYPE', 'esp32')
network = None
if MCU_TYPE == "esp32":
    try:
        import network
    except ImportError:
        pass  # If import fails even on esp32, network remains None
...
async def connectToWifiNetwork():
    s = get_settings()
    if MCU_TYPE == "pico" or network is None:
        sdata.WIFI_CONNECTED = False
        await debug_print("WiFi not supported on this MCU type.", "WIFI")
        return False
...
async def scanToWifiNetwork():
    if MCU_TYPE == "pico" or network is None:
        await debug_print("WiFi scan not supported on this MCU type.", "WIFI")
        return []
...
async def showNetworkWIFI():
    if MCU_TYPE == "pico" or network is None:
        try:
            from oled import display_message
            await display_message("WiFi: Not supported", 1.8)
        except Exception:
            pass
        await debug_print("WiFi show not supported on this MCU type.", "WIFI")
        return
...
async def check_internet_connection():
    if MCU_TYPE == "pico" or network is None:
        await debug_print("Internet check not supported on this MCU type.", "WIFI")
        return False
    # ...existing code...
    try:
