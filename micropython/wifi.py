# /workspaces/TMON/micropython/wifi.py
...
def _wifi_status_name(status_code):
    try:
        if network is None:
            return "unknown"
        mapping = {
            ...
            getattr(network, 'STAT_NO_AP_FOUND', -2): "ap not found",
            getattr(network, 'STAT_CONNECT_FAIL', -1): "connect failed",
            getattr(network, 'STAT_GOT_IP', 3): "got ip",
        }
        return mapping.get(status_code, "unknown")
    except Exception:
        return "unknown"

def _wifi_is_fail_status(status_code):
    try:
        if network is None:
            return False
...
async def connectToWifiNetwork():
    ...
    require_scan = bool(getattr(s, 'WIFI_REQUIRE_SCAN_MATCH', False))
    if require_scan and not target_found:
        await debug_print("wifi: ssid not found (scan-required mode)", "WARN")
        try:
            from oled import display_message
        ...
        await debug_print("wifi: ssid not seen in scan; trying direct connect", "WARN")
    ...
    poll_interval_s = 1
    last_status = None
    ssid = getattr(s, 'WIFI_SSID', '')
    password = getattr(s, 'WIFI_PASS', '')

    await debug_print(f"wifi: connect {ssid}", "WIFI")
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            await debug_print(f"wifi: attempt {attempt}/{retries}", "WIFI")

            start_ms = time.ticks_ms()
            while time.ticks_diff(time.ticks_ms(), start_ms) < (connect_timeout_s * 1000):
                if wlan.isconnected():
                    sdata.WIFI_CONNECTED = True
                    try:
                        s.net_wifi_MAC = wlan.config('mac')
                    ...
                    try:
                        ...
                        pass
                    _refresh_rssi(wlan)
                    await debug_print("wifi: connected", "WIFI")
                    try:
                        await debug_print(f"IP: {getattr(s, 'net_wifi_IP', None)}, MAC: {getattr(s, 'net_wifi_MAC', None)}", "WIFI")
                    except Exception:
                        pass
                    return True
                ...
                if status != last_status:
                    last_status = status
                    await debug_print(f"wifi: status={status} ({_wifi_status_name(status)})", "WIFI")

                if _wifi_is_fail_status(status):
                    break

                await asyncio.sleep(poll_interval_s)

            status = wlan.status()
            await debug_print(f"wifi: attempt {attempt} failed status={status} ({_wifi_status_name(status)})", "WARN")
    ...
    try:
        status = wlan.status()
        await debug_print(f"wifi: connect failed status={status} ({_wifi_status_name(status)})", "ERROR")
    except Exception:
...
async def check_internet_connection():
    ...
    try:
        ...
        # Prefer a lightweight connectivity endpoint; accept common success/redirect statuses.
        response = urequests.get("http://connectivitycheck.gstatic.com/generate_204")
        try:
            code = int(getattr(response, 'status_code', 0) or 0)
            if code in (200, 204, 301, 302):
                sdata.WAN_CONNECTED = True
                await debug_print(f"Internet access verified (HTTP {code}).", "WIFI")
                try:
                    response.close()
                except Exception:
                    pass
                return True
            await debug_print(f"WiFi connected but WAN probe returned HTTP {code}.", "WARN")
            try:
                response.close()
            except Exception:
                pass
            ...
            try:
                ...
                pass
            sdata.WAN_CONNECTED = False
            await debug_print("Connected to WiFi but WAN probe parse failed.", "WARN")
            return False
    except Exception:
...
async def disable_wifi():
    ...
    sdata.WIFI_CONNECTED = False
    try:
        get_settings().ENABLE_WIFI = False
    except Exception:
        pass
