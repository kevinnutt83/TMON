...
jQuery(document).ready(function($){
	...
	function getUnitIdFromRow($btn) {
		const $tr = $btn.closest('tr');
		// Table shown in prompt: Unit ID is the 2nd <td>
		const txt = ($tr.find('td').eq(1).text() || '').trim();
		return txt;
	}
	...
	$(document).on('click', '.tmon-relay-btn', function (e) {
		e.preventDefault();

		const $btn = $(this);
		const relay = String($btn.data('relay') || '').trim();
		const state = String($btn.data('state') || '').trim();
		const unitId = getUnitIdFromRow($btn);

		if (!unitId || !relay || !state) return;

		setLoading($btn, true);

		$.post(
			(window.TMON_UC && TMON_UC.ajaxUrl) ? TMON_UC.ajaxUrl : ajaxurl,
			{
				action: 'tmon_uc_toggle_relay',
				nonce: (window.TMON_UC && TMON_UC.nonce) ? TMON_UC.nonce : '',
				unit_id: unitId,
				relay: relay,
				state: state,
				runtime_s: '0'
			...
			.done(function (resp) {
				if (!resp || !resp.success) {
					...
					console.error('Relay toggle failed', resp);
					return;
				}
				// Optional: you can refresh the row state here once you expose relay state via REST.
				// eslint-disable-next-line no-console
				console.log('Relay command queued', resp.data);
			})
			.fail(function (xhr) {
				// eslint-disable-next-line no-console
				console.error('Relay toggle AJAX error', xhr && xhr.responseText);
			})
			.always(function () {
