...
function tmon_uc_purge_all_core() {
    global $wpdb;
    // Delete DB rows (primary tables)
    ...
    // UC mirror & staged table
    $wpdb->query("DELETE FROM {$wpdb->prefix}tmon_uc_devices");
    $wpdb->query("DELETE FROM {$wpdb->prefix}tmon_staged_settings");

    // Clear legacy/options maps and pairing data used by shortcodes/blocks
    delete_option('tmon_admin_provisioned_devices');
    delete_option('tmon_uc_staged_settings');
    delete_option('tmon_uc_paired_sites');
    delete_option('tmon_uc_admin_key');
    delete_option('tmon_uc_hub_shared_key');
    delete_option('tmon_uc_hub_read_token');

    // Delete log/csv files
    $dir = WP_CONTENT_DIR . '/tmon-field-logs';
    if (is_dir($dir)) {
        foreach (glob($dir . '/*') as $f) { @unlink($f); }
    }

    // Let other parts of the plugin/themes clear caches/refresh shortcodes
    do_action('tmon_uc_purged_all');

    return true;
}

function tmon_uc_purge_unit_core($unit_id) {
    global $wpdb;
    ...
    // UC mirror and staged table entries
    $wpdb->delete($wpdb->prefix.'tmon_uc_devices', ['unit_id' => $unit_id]);
    $wpdb->delete($wpdb->prefix.'tmon_staged_settings', ['unit_id' => $unit_id]);

    // Remove from option-based staged map if present
    $map = get_option('tmon_uc_staged_settings', []);
    if (is_array($map) && isset($map[$unit_id])) {
        unset($map[$unit_id]);
        update_option('tmon_uc_staged_settings', $map);
    }

    // Remove files for this unit
    $dir = WP_CONTENT_DIR . '/tmon-field-logs';
    ...
    do_action('tmon_uc_purged_unit', $unit_id);
...
add_action('admin_post_tmon_uc_purge_all', function(){
    ...
    tmon_uc_purge_all_core();

    wp_safe_redirect(admin_url('admin.php?page=tmon-settings&purge=all'));
    exit;
});

// Replace admin_post unit purge to use helper (keeps behaviour and redirects the same)
add_action('admin_post_tmon_uc_purge_unit', function(){
    ...
    if ($unit_id) {
        tmon_uc_purge_unit_core($unit_id);
    }
    wp_safe_redirect(admin_url('admin.php?page=tmon-settings&purge=unit'));
    exit;
});

// REST: Purge endpoints (for automation or central admin) — use helpers and consistent response
...
add_action('admin_post_tmon_uc_pair_with_hub', function(){
    ...
    if (isset($res['status']) && $res['status'] === 'ok' && !empty($res['paired'])) {
        ...
        wp_safe_redirect(admin_url('admin.php?page=tmon-uc-hub&paired=0&msg=' . urlencode($msg)));
    }
    exit;
});
...
// Pair with hub: persist keys and normalized pairing; backfill devices to UC cache (fallback redirect handler)
add_action('admin_post_tmon_uc_pair_with_hub', function(){
    if (!current_user_can('manage_options')) wp_die('Insufficient permissions');
    check_admin_referer('tmon_uc_pair_with_hub');
    $res = tmon_uc_pair_with_hub_core();
...
// Expose settings schema to admin JS for dynamic settings editor
add_action('wp_ajax_tmon_uc_get_settings_schema', function() {
	if (! current_user_can('manage_options')) {
		wp_send_json_error('forbidden', 403);
	}
	// nonce check (optional; script sends tmon_uc_nonce as 'nonce' param)
	if ( isset($_REQUEST['nonce']) ) {
		check_ajax_referer('tmon_uc_nonce', 'nonce');
	}
	$schema = function_exists('tmon_uc_settings_schema') ? tmon_uc_settings_schema() : array();
