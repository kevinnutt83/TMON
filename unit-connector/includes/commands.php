...
// Command queue (option-backed) for firmware polling behavior
if (!defined('TMON_UC_CMDQ_CAP')) {
	define('TMON_UC_CMDQ_CAP', 50);
}

function tmon_uc_cmdq_option_name(string $unit_id): string {
	$unit_id = preg_replace('/[^a-zA-Z0-9\-_]/', '', $unit_id);
	return 'tmon_uc_cmdq_' . strtolower($unit_id);
}

function tmon_uc_enqueue_command(string $unit_id, string $command, array $args = []) {
	$unit_id = trim($unit_id);
	$command = trim($command);
	if ($unit_id === '' || $command === '') return false;

	$opt = tmon_uc_cmdq_option_name($unit_id);
	$q = get_option($opt, []);
	if (!is_array($q)) $q = [];

	$cmd_id = 'cmd_' . wp_generate_uuid4();
	$q[] = [
		'id'         => $cmd_id,
		'created_at' => time(),
		'command'    => $command,
	...
	update_option($opt, $q, false);
	return $cmd_id;
}

function tmon_uc_peek_next_command(string $unit_id) {
	$q = get_option(tmon_uc_cmdq_option_name($unit_id), []);
	if (!is_array($q) || empty($q)) return null;
	return $q[0];
}

function tmon_uc_dequeue_next_command(string $unit_id) {
	$opt = tmon_uc_cmdq_option_name($unit_id);
	$q = get_option($opt, []);
	if (!is_array($q) || empty($q)) return null;
...
function tmon_uc_mark_command_complete(string $unit_id, string $command_id, array $result = []): bool {
	// Keep a small rolling completion log for debugging/support.
	$log_opt = 'tmon_uc_cmdlog_' . strtolower(preg_replace('/[^a-zA-Z0-9\-_]/', '', $unit_id));
	$log = get_option($log_opt, []);
	if (!is_array($log)) $log = [];
	...
	update_option($log_opt, $log, false);
	return true;
}
