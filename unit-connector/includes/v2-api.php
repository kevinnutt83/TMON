...
// Allow TMON Admin to push a device sync/update to this UC site.
// Authenticated via tmon_uc_rest_permission_check (accepts hub/admin token or Authorization).
add_action('rest_api_init', function () {
    register_rest_route( 'tmon/v1', '/admin/device/sync', array(
        'methods' => 'POST',
        'callback' => function( $request ) {
            $auth = tmon_uc_rest_permission_check($request);
            if (is_wp_error($auth)) return $auth;
            $payload = $request->get_json_params();
            if (!is_array($payload)) return new WP_Error('bad_payload','Invalid payload', array('status'=>400));
            $dev = $payload['device'] ?? null;
            if (!is_array($dev) || empty($dev['unit_id'])) return new WP_Error('bad_device','Missing device unit_id', array('status'=>400));
            global $wpdb;
            $table = $wpdb->prefix . 'tmon_devices';
            $unit = sanitize_text_field($dev['unit_id']);
            $machine = sanitize_text_field($dev['machine_id'] ?? '');
            $name = sanitize_text_field($dev['unit_name'] ?? '');
            $role = sanitize_text_field($dev['role'] ?? '');
            // Upsert basic record
            $exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$table} WHERE unit_id=%s", $unit));
            if ($exists) {
                $wpdb->update($table, array('machine_id'=>$machine,'unit_name'=>$name,'role'=>$role,'last_seen'=>current_time('mysql')), array('unit_id'=>$unit));
            } else {
                $wpdb->insert($table, array('unit_id'=>$unit,'machine_id'=>$machine,'unit_name'=>$name,'role'=>$role,'last_seen'=>current_time('mysql')));
            }
            // Optionally store latest telemetry snapshot if provided
            if (!empty($dev['last_field_data'])) {
                $fd_table = $wpdb->prefix . 'tmon_field_data';
                $wpdb->insert($fd_table, array('unit_id'=>$unit,'data'=>wp_json_encode($dev['last_field_data']),'created_at'=>current_time('mysql')));
            }
            return rest_ensure_response(array('status'=>'ok','unit_id'=>$unit));
        },
        'permission_callback' => 'tmon_uc_rest_permission_check',
