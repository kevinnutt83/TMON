...
function tmon_hierarchy_page() {
    ...
    if (!current_user_can('manage_options') && !current_user_can('manage_tmon') && !current_user_can('edit_tmon_hierarchy')) {
        wp_die('Forbidden');
    }
    ...
    echo '</div>';
    echo '<script>window.TMON_LEAFLET_READY = !!window.L;</script>';
    echo '<script>if (!window.TMON_LEAFLET_READY) { document.write("<script src=\\\"https://unpkg.com/leaflet@1.9.4/dist/leaflet.js\\\"></"+"script>"); }</script>';
