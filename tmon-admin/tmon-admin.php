...
add_action('admin_post_tmon_admin_save_provision', function() {
    ...
    if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $dev_table))) {
        ...
        if (in_array('provisioned', $dev_cols) || in_array('status', $dev_cols)) {
            // After earlier update upserting provisioned row, mirror changes
            $mirror_update = [
            ...
            if (!empty($unit_id)) {
                ...
                error_log('tmon-admin: mirror update for unit_id=' . $unit_id);
            } elseif (!empty($machine_id)) {
                $wpdb->update($dev_table, $mirror_update, ['machine_id' => $machine_id]);
                error_log('tmon-admin: mirror update for machine_id=' . $machine_id);
            } else {
                error_log('tmon-admin: save_provision called without unit_id or machine_id; no mirror update.');
            }
        }
    }
