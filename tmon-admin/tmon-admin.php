...
// Add guard: ensure Provisioning table exists to avoid fatal on admin page.
// This is minimal to prevent errors; full schema management should live in includes/db.php.
...
// Note: UC cron error shows its device commands table is missing a `status` column.
// That belongs to the UC plugin schema (mp_tmon_device_commands). Fix in UC plugin.

// Remove repeated duplicate helper/page blocks that appear after the truncation.
// Keep only the first occurrence of each earlier definition:
// - tmon_admin_normalize_key
// - tmon_admin_uc_normalize_url
// - tmon_admin_uc_pairings_get / set
// - tmon_admin_uc_key_generate
// - tmon_admin_hmac_valid
// - tmon_admin_pairings_page
// - tmon_admin_firmware_page
// - tmon_admin_provisioning_history_page
// - update_option_tmon_admin_repo hook
// - TMON_ADMIN_HANDLERS_INCLUDED fallback
...
if (!function_exists('tmon_admin_pairings_page')) {
	function tmon_admin_pairings_page() {
		...
		if (empty($pairings)) {
			...
			echo '</tr></thead><tbody>';
			foreach ($pairings as $key_id => $info) {
				$status = !empty($info['active']) ? 'Active