...
function tmon_admin_render_ota_jobs_and_actions(){
	...
	echo '<div class="tmon-card"><h2>OTA Jobs</h2>';
	echo '<table class="wp-list-table widefat striped"><thead><tr><th>ID</th><th>Unit</th><th>Action</th><th>Args</th><th>Status</th><th>Created</th><th>Updated</th></tr></thead><tbody>';
	foreach ($rows as $r){
	...
	echo '<div class="tmon-card"><h2>Device Actions</h2>';
	echo '<p><em>Select a device and issue commands: download logs, reboot, update firmware, clear logs (preserve settings).</em></p>';
	// Provide simple form to send commands (wire to existing AJAX)
	echo '<form method="post" action="'.esc_url(admin_url('admin-ajax.php')).'"><input type="hidden" name="action" value="tmon_send_device_command" />';
	echo '<p><label>Unit ID <input name="unit_id" required></label> <label>Command <select name="command"><option value="download_logs">download_logs</option><option value="reboot">reboot</option><option value="firmware_update">firmware_update</option><option value="clear_logs">clear_logs</option></select></label> <button class="button button-primary">Send</button></p></form>';
	echo '</div>';
}
