...
if (!function_exists('tmon_admin_install_schema')) {
	function tmon_admin_install_schema() {
		...
		// Defensive: ensure columns exist on legacy installs.
		tmon_admin_ensure_columns($devices, [
			...
			'last_seen'   => "ALTER TABLE {$devices} ADD COLUMN last_seen DATETIME NULL DEFAULT NULL",
			'settings'    => "ALTER TABLE {$devices} ADD COLUMN settings LONGTEXT NULL",
		]);
		...
		// Allow add-on modules (provisioning, claims, etc.) to install their schemas.
		do_action('tmon_admin_install_schema_after');
	}
}

// Keep utility guarded to avoid accidental re-declare.
