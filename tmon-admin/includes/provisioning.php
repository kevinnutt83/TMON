...
function tmon_admin_provisioning_page() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && function_exists('tmon_admin_verify_nonce') && tmon_admin_verify_nonce('tmon_admin_provision')) {
        ...
        // Ensure prov table variable is available
        $prov_table = $wpdb->prefix . 'tmon_provisioned_devices';
        ...
        if ($action === 'update') {
            ...
            // Compute normalized or fallback ids for the row; prefer POST values then row fallback
            ...
            $role = sanitize_text_field($_POST['role'] ?? ($row_tmp['role'] ?? 'base'));
            $plan = sanitize_text_field($_POST['plan'] ?? ($row_tmp['plan'] ?? 'standard'));
            $status = sanitize_text_field($_POST['status'] ?? ($row_tmp['status'] ?? 'active'));
            $company_id = intval($_POST['company_id'] ?? ($row_tmp['company_id'] ?? 0));
            $notes = sanitize_textarea_field($_POST['notes'] ?? ($row_tmp['notes'] ?? ''));
            $site_url = esc_url_raw($_POST['site_url'] ?? ($row_tmp['site_url'] ?? ''));
            $firmware = sanitize_text_field($_POST['firmware'] ?? ($row_tmp['firmware'] ?? ''));
            $firmware_url = esc_url_raw($_POST['firmware_url'] ?? ($row_tmp['firmware_url'] ?? ''));

            // Make sure $exists is defined for both create and update paths
            $exists = $wpdb->get_var($wpdb->prepare("SELECT id FROM {$prov_table} WHERE unit_id=%s AND machine_id=%s", $unit_id, $machine_id));

            // Insert if id==0 (create) else update
            if ($id === 0) {
                // Build fields for insert
                $fields = [
                    'unit_id' => $unit_id,
                    ...
                    'settings_staged' => $do_provision ? 1 : 0,
                ...
                // Update fields for existing row (id != 0)
                $fields = [
                    'role' => $role,
                    'plan' => $plan,
                    'status' => $status,
            ...
            // If staged update was requested, set the staged flags in DB using normalized columns (if available)
            if ($do_provision) {
                if (!empty($mac_norm)) {
            ...
            // If Save & Provision clicked, enqueue & mirror
            if ($do_provision) {
                ...
                if ($row_id) {
                    ...
                    // Enqueue for both machine and unit
                    if (!empty($machine_id)) {
                    ...
                    $mirror_dev_table = $wpdb->prefix . 'tmon_devices';
                    if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $mirror_dev_table))) {
                        $dev_cols = $wpdb->get_col("SHOW COLUMNS FROM {$mirror_dev_table}");
                        ...
                        if (!empty($unit_id)) $wpdb->update($mirror_dev_table, $mirror, ['unit_id' => $unit_id]);
                        elseif (!empty($machine_id)) $wpdb->update($mirror_dev_table, $mirror, ['machine_id' => $machine_id]);
                    }
            ...
            // Update done, redirect to avoid re-post on refresh
            wp_redirect(add_query_arg('provision', 'success', $redirect_url));
            exit;
        ...
        // DELETE action
            ...
            if ($unit_id && $machine_id) {
                ...
                if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $dev_table))) {
                    ...
                    wp_redirect(add_query_arg('provision', $deleted ? 'success' : 'fail', $redirect_url));
                    exit;
                }
            }
            wp_redirect(add_query_arg('provision', 'fail', $redirect_url));
            exit;

        // Push configuration to UC
...
add_action('admin_post_tmon_admin_provision_device', function() {
    ...
    if ($save_provision) {
        ...
        if ($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $dev_table))) {
            ...
            if (in_array('provisioned', $dev_cols)) $mirror['provisioned'] = 1; else $mirror['status'] = 'provisioned';
            if (in_array('site_url', $dev_cols)) $mirror['site_url'] = $payload['site_url'];
            if (in_array('wordpress_api_url', $dev_cols)) $mirror['wordpress_api_url'] = $payload['site_url'];
            if (in_array('unit_name', $dev_cols)) $mirror['unit_name'] = $payload['unit_name'];
            if (in_array('provisioned_at', $dev_cols)) $mirror['provisioned_at'] = current_time('mysql');
