...
function tmon_unit_connector_enqueue_assets($hook = '') {
    if (is_admin()) {
        ...
        wp_enqueue_script('tmon-hierarchy-js', plugin_dir_url(__FILE__) . 'assets/tmon-hierarchy.js', array('jquery'), TMON_UNIT_CONNECTOR_VERSION, true);
...
add_filter('site_transient_update_plugins', function($transient) {
    ...
    if (version_compare($plugin_data['Version'], ltrim($release->tag_name, 'v'), '<')) {
        $transient->response[$plugin_slug] = (object) [
            ...
            'package' => (!empty($release->assets) && isset($release->assets[0]->browser_download_url)) ? $release->assets[0]->browser_download_url : '',
        ];
    }
    return $transient;
});
