...
function tmon_admin_install_schema() {
    ...
    // Hierarchy tables (company/site/zone/cluster/unit) for redundancy on fresh installs
...
add_action('wp_ajax_tmon_admin_known_units', function(){
    ...
    // Validate search query: max 64 chars, only allow alphanum, dash, colon, space
    if (strlen($q) > 64 || !preg_match('/^[\w\s\-:]{0,64}$/u', $q)) {
        wp_send_json_error(['message' => 'invalid_query'], 400);
    }
